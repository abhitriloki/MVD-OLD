...
const Profile = (props) => {
    ...
    const call_get_profile = () => {
        ...
        .then(async response => {
            ...
            props.updateAadharNumber(response.data.result.aadhar_number);
            setOnLoad(1);
        })
        .catch(error => {
            setLoading(false);
    ...
    const call_profile_picture_update = async (data) => {
        ...
        .then(async response => {
            ...
            if (response.data.status == 1) {
                alert(strings.your_profile_picture_update_successfully)
                call_get_profile();
            } else {
        ...
        <SafeAreaView style={{ backgroundColor: colors.lite_bg, flex: 1 }}>
            ...
            <View style={[styles.header]}>
                ...
                <View activeOpacity={1} style={{ width: '85%', alignItems: 'flex-start', justifyContent: 'center' }}>
                    <Text numberOfLines={1} ellipsizeMode='tail' style={{ color: colors.theme_fg_three, fontSize: f_xl, fontFamily: bold }}>{strings.edit_profile}</Text>
                </View>
            </View>
            <ScrollView>
                {on_load == 1 &&
                    <View>
                        ...
                        <ScrollView>
                            <View style={{ alignItems: 'center' }}>
                                <Text style={{ color: colors.theme_fg_two, fontSize: f_xl, fontFamily: bold }}>{strings.say_a_little_bit_about_yourself}</Text>
                                <View style={{ margin: 20 }} />
                                <View style={{ width: '90%' }}>
                                    <View style={{ marginBottom: 20 }}>
                                        <Text style={{ color: colors.text_grey, fontSize: f_xs, fontFamily: bold }}>{strings.first_name}</Text>
                                        <View style={{ margin: 5 }} />
                                    ...
                                    <View style={{ marginBottom: 20 }}>
                                        <Text style={{ color: colors.text_grey, fontSize: f_xs, fontFamily: bold }}>{strings.last_name}</Text>
                                        <View style={{ margin: 5 }} />
                                    ...
                                    <View style={{ marginBottom: 20 }}>
                                        <Text style={{ color: colors.text_grey, fontSize: f_xs, fontFamily: bold }}>{strings.phone_number}</Text>
                                        <View style={{ margin: 5 }} />
