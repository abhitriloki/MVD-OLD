...
import {
  ...
  LONGITUDE_DELTA, f_s, f_tiny, f_xs, get_heatmap_coordinates
...
const Dashboard = (props) => {
  ...
  const [online_message, setOnlineMessage] = useState("");
  const [online_message_status, setOnlineMessageStatus] = useState(0);
  ...
  const call_change_online_status = async (status) => {
    ...
    await axios({
      ...
      .then(async response => {
        setLoading(false);
        setOnlineMessage(response.data.message);
        setOnlineMessageStatus(response.data.status);
  ...
  const get_background_location_permission = async () => {
    const bg_granted = await PermissionsAndroid.request(
      ...
      'title': app_name + strings.app_access_your_location_for_tracking_the_background,
      'message': strings.access_your_location_for_tracking_in_background,
      buttonPositive: "OK"
    }
    )
  }
  ...
  const requestCameraPermission = async () => {
    try {
      const granted = await PermissionsAndroid.request(
        ...
        'title': app_name + strings.app_access_your_location_for_tracking_in_background,
        'message': strings.access_your_location_for_tracking_in_background
      }
      )
  ...
  const get_location = async (vt, sy) => {
    const granted = await PermissionsAndroid.request(
      PermissionsAndroid.PERMISSIONS.ACCESS_FINE_LOCATION, {
      title: { app_name } + strings.access_your_location_in_background_for_get_nearest_trip_requests,
      message: { app_name } + strings.needs_to_access_your_location_in_background_for_get_nearest_trips_show_live_location_to_patients_that_will_be_always_in_use
    }
    );
    if (granted && vt != 0) {
    ...
    <View style={styles.container}>
      ...
      <View style={{ padding: 15, backgroundColor: colors.theme_bg_three, flexDirection: 'row', position: 'absolute', top: 20, width: '90%', marginLeft: '5%', borderRadius: 10 }}>
        ...
        <View style={{ width: '60%', alignItems: 'center', justifyContent: 'center' }}>
          {switch_value == true &&
            <Text style={{ color: colors.theme_fg, fontSize: f_s, fontFamily: bold }}>{strings.online}</Text>
          }
          {switch_value != true &&
            <Text style={{ color: colors.theme_fg_two, fontSize: f_s, fontFamily: bold }}>{strings.offline}</Text>
          }
        </View>
      ...
      {online_message_status != 1 &&
        <View style={{ padding: 10, height: 110, position: 'absolute', top: 10, bottom: 0, width: '90%', borderRadius: 10, alignSelf: 'center', justifyContent: 'center' }}>
          <Text numberOfLines={3} style={{ color: colors.theme_fg, fontSize: f_s, fontFamily: bold, textAlign: 'center' }}>{online_message}</Text>
        </View>
      }
      <View style={{ padding: 10, backgroundColor: colors.theme_bg, height: 120, position: 'absolute', bottom: 80, width: '90%', marginLeft: '5%', borderRadius: 10 }}>
        <DropShadow
          style={{
            width: '100%',
            marginBottom: 5,
        ...
        >
          <View style={{ flexDirection: 'row', width: '100%' }}>
            ...
            <View style={{ width: '50%', alignItems: 'center', justifyContent: 'center' }}>
              <Icon type={Icons.FontAwesome} name="rupee" style={{ fontSize: 30, color: colors.theme_fg_three }} />
              <View style={{ margin: 5 }} />
          ...
          <View style={{ flexDirection: 'row', width: '100%' }}>
            <View style={{ width: '50%', alignItems: 'center', justifyContent: 'center' }}>
              <Text style={{ color: colors.theme_fg_three, fontSize: f_tiny, fontFamily: normal }}>{strings.today_bookings}</Text>
            </View>
            <View style={{ width: '50%', alignItems: 'center', justifyContent: 'center' }}>
              <Text style={{ color: colors.theme_fg_three, fontSize: f_tiny, fontFamily: normal }}>{strings.today_earnings}</Text>
            </View>
          </View>
        </DropShadow>
      </View>
      ...
      <View style={{ position: 'absolute', top: 90, width: '100%' }}>
        {pending_hire_bookings > 0 &&
          <TouchableOpacity activeOpacity={1} onPress={navigate_rental.bind(this)} style={{ flexDirection: 'row', backgroundColor: colors.success_background, borderRadius: 10, alignItems: 'center', justifyContent: 'center', padding: 10, width: '90%', marginLeft: '5%', marginBottom: 10 }}>
            <Icon type={Icons.Ionicons} name="bookmark" style={{ fontSize: 20, color: colors.success }} />
            <View style={{ margin: 5 }} />
            <Text style={{ fontFamily: regular, fontSize: f_xs, color: colors.success }}>{strings.you_have_received_driver_hire_request}</Text>
          </TouchableOpacity>
        }
        {/* {wallet == 0 &&
