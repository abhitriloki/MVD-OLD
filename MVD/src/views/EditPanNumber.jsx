...
const EditPanNumber = (props) => {
    ...
    const check_valid = () => {
        if (pan_number) {
            ...
            dropDownAlertRef.alertWithType('error', strings.validation_error, strings.please_enter_your_pan_number);
        }
    }
    ...
    const call_update_kyc = () => {
        ...
        axios({
            ...
            .then(async response => {
                setLoading(false);
                dropDownAlertRef.alertWithType('success', strings.successfully_updated, strings.your_pan_number_has_been_updated);
                go_back();
            })
            .catch(error => {
                setLoading(false);
    ...
    return (
        <SafeAreaView style={{ backgroundColor: colors.lite_bg, flex: 1 }}>
            ...
            <View style={{ alignItems: 'center', justifyContent: 'center' }}>
                <Text numberOfLines={1} style={{ color: colors.theme_fg_two, fontSize: f_xl, fontFamily: bold }}>{strings.enter_your_pan_number}</Text>
                <View style={{ margin: 5 }} />
                <Text numberOfLines={1} style={{ color: colors.grey, fontSize: f_xs, fontFamily: normal }}>{strings.you_need_enter_your_pan_number}</Text>
                <View style={{ margin: 20 }} />
                <View style={{ width: '80%' }}>
