...
console.log(route.params.otp)
    ...
    const check_valid = (val) => {
        if (val != otp) {
            dropDownAlertRef.alertWithType('error', strings.validation_error, strings.please_enter_valid_otp);
        } else {
            navigate();
        }
    }
    ...
    return (
        <SafeAreaView style={{ backgroundColor: colors.lite_bg, flex: 1 }}>
            ...
            <View style={{ alignItems: 'center', justifyContent: 'center' }}>
                <Text numberOfLines={1} style={{ color: colors.theme_fg_two, fontSize: f_xl, fontFamily: bold }}>{strings.enter_the_otp}</Text>
                <View style={{ margin: 5 }} />
                <Text numberOfLines={1} style={{ color: colors.grey, fontSize: f_xs, fontFamily: normal }}>{strings.enter_your_otp_received_from_the_sms}</Text>
                <View style={{ margin: 20 }} />
                <View style={{ width: '80%' }}>
