...
const Bookings = (props) => {
    ...
    const navigate = (trip_id, status_name) => {
        if (filter == 1) {
            ...
            dropDownAlertRef.alertWithType('error',strings.error, strings.this_trip_is + ' ' + status_name);
        }
    }
                        ...
                        >
                            <View style={{ flexDirection: 'row', flex: 1, backgroundColor: colors.theme_bg_three, padding: 15, borderTopLeftRadius: 10, borderTopRightRadius: 10 }}>
                                ...
                                <View style={{ width: '25%', alignItems: 'center', justifyContent: 'center' }}>
                                    <Text style={{ color: colors.text_grey, fontSize: f_xs, fontFamily: normal }}>{strings.fare}</Text>
                                    <View style={{ margin: 3 }} />
                                    <Text style={{ fontSize: f_s, fontFamily: bold, color: colors.theme_fg_two }}>{global.currency}{item.total}</Text>
                                </View>
                                <View style={{ width: '25%', alignItems: 'center', justifyContent: 'center' }}>
                                    <Text style={{ color: colors.text_grey, fontSize: f_xs, fontFamily: normal }}>{strings.distance}</Text>
                                    <View style={{ margin: 3 }} />
        ...
        <SafeAreaView style={{ backgroundColor: colors.lite_bg, flex: 1 }}>
            ...
            <View style={[styles.header]}>
                ...
                <TouchableOpacity activeOpacity={1} onPress={navigate_home.bind(this)} style={{ width: '85%', alignItems: 'flex-start', justifyContent: 'center' }}>
                    <Text numberOfLines={1} ellipsizeMode='tail' style={{ color: colors.theme_fg_two, fontSize: f_xl, fontFamily: bold }}>{strings.my_rides}</Text>
                </TouchableOpacity>
            </View>
                    ...
                    >
                        <View style={{ flexDirection: 'row', backgroundColor: colors.theme_bg_three, borderRadius: 10 }}>
                            <TouchableOpacity onPress={change_filter.bind(this, 1)} style={[filter == 1 ? styles.segment_active_bg : styles.segment_inactive_bg]}>
                                <Text style={[filter == 1 ? styles.segment_active_fg : styles.segment_inactive_fg]}>{strings.on_going}</Text>
                            </TouchableOpacity>
                            <TouchableOpacity onPress={change_filter.bind(this, 2)} style={[filter == 2 ? styles.segment_active_bg : styles.segment_inactive_bg]}>
                                <Text style={[filter == 2 ? styles.segment_active_fg : styles.segment_inactive_fg]}>{strings.completed}</Text>
                            </TouchableOpacity>
                            <TouchableOpacity onPress={change_filter.bind(this, 3)} style={[filter == 3 ? styles.segment_active_bg : styles.segment_inactive_bg]}>
                                <Text style={[filter == 3 ? styles.segment_active_fg : styles.segment_inactive_fg]}>{strings.cancelled}</Text>
                            </TouchableOpacity>
                        </View>
                    </DropShadow>
                </View>
...
const styles = StyleSheet.create({
    ...
    segment_active_bg: { width: '34%', alignItems: 'center', justifyContent: 'center', padding: 15, backgroundColor: colors.theme_bg, borderRadius: 10 },
