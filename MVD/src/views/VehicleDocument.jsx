...
const VehicleDocument = (props) => {
  ...
  const [rejected_reasons, setRejectedReasons] = useState(false);
  ...
  const call_get_documents = async () => {
    ...
    console.log({ driver_id: global.id, lang: global.lang });
    setLoading(true);
    await axios({
      method: 'post',
      ...
      .then(async response => {
        ...
        setRejectedReasons(response.data.result.rejected_reasons);
        console.log(response.data.result.rejected_reasons)
      })
      .catch(error => {
        console.log(error);
        setLoading(false);
        ...
        <View style={{ padding: 10 }}>
          ...
          {rejected_reasons != " " &&
            <View style={{backgroundColor:colors.error_background,alignItems:'center',justifyContent:'center',padding:5}}>
              <Text style={{ fontFamily: normal, color: colors.error, fontSize: f_xs }}>{strings.note} : {rejected_reasons}</Text>
            </View>
          }
          <View style={{ margin: 5 }} />
          <View>
            ...
            <TouchableOpacity onPress={move_to_upload.bind(this, 'id_proof', id_proof.status, id_proof.path)} activeOpacity={1} style={{ borderWidth: 1, padding: 10, borderRadius: 5, borderStyle: 'dashed', flexDirection: 'row' }}>
              <View style={{ width: '70%' }}>
                ...
                <View style={{ margin: 5 }} />
                <Text style={{ fontFamily: normal, color: colors.grey, fontSize: f_xs }}>{strings.upload_your_driving_license}</Text>
              </View>
          ...
          <View>
            <Text style={{ fontFamily: bold, color: colors.theme_fg_two, fontSize: f_l }}>
              {strings.registration_certificate}
            </Text>
            <Text style={{ fontFamily: normal, color: colors.grey, fontSize: f_xs }}>
              {strings.make_sure_that_every_details_of_the_document_is_clearly_visible}
            </Text>
            ...
            <TouchableOpacity onPress={move_to_upload.bind(this, 'vehicle_certificate', vehicle_certificate.status, vehicle_certificate.path)} activeOpacity={1} style={{ borderWidth: 1, padding: 10, borderRadius: 5, borderStyle: 'dashed', flexDirection: 'row' }}>
              <View style={{ width: '70%' }}>
                ...
                <View style={{ margin: 5 }} />
                <Text style={{ fontFamily: normal, color: colors.grey, fontSize: f_xs }}>{strings.upload_your_vahan_registration_certificate}</Text>
              </View>
              <View style={{ width: '30%', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <View>
            ...
            <TouchableOpacity onPress={move_to_upload.bind(this, 'vehicle_insurance', vehicle_insurance.status, vehicle_insurance.path)} activeOpacity={1} style={{ borderWidth: 1, padding: 10, borderRadius: 5, borderStyle: 'dashed', flexDirection: 'row' }}>
              <View style={{ width: '70%' }}>
                ...
                <View style={{ margin: 5 }} />
                <Text style={{ fontFamily: normal, color: colors.grey, fontSize: f_xs }}>{strings.upload_your_vahan_insurance_document}</Text>
              </View>
              <View style={{ width: '30%', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <View>
            ...
            <Text style={{ fontFamily: normal, color: colors.grey, fontSize: f_xs }}>
              {strings.upload_your_vahan_image}
            </Text>
            <View style={{ margin: 10 }} />
            <TouchableOpacity onPress={move_to_upload.bind(this, 'vehicle_image', vehicle_image.status, vehicle_image.path)} activeOpacity={1} style={{ borderWidth: 1, padding: 10, borderRadius: 5, borderStyle: 'dashed', flexDirection: 'row' }}>
              <View style={{ width: '70%' }}>
                ...
                <View style={{ margin: 5 }} />
                <Text style={{ fontFamily: normal, color: colors.grey, fontSize: f_xs }}>{strings.upload_your_vahan_image_with_number_board}</Text>
                {/* {vehicle_image.status == 17 &&
                  <View>
                    <View style={{ margin: 5 }} />
                    <Text style={{ fontFamily: normal, color: colors.error, fontSize: f_xs }}>{strings.note} : License number not visible</Text>
                  </View>
                } */}
              </View>
              <View style={{ width: '30%', alignItems: 'center', justifyContent: 'center' }}>
