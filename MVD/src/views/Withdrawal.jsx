...
const Withdrawal = (props) => {
  ...
  const sendInput = (val) => {
    ...
    if (isNaN(val) || val == 0 || val == null) {
      dropDownAlertRef.alertWithType('error', 'Validation error', 'Please enter valid amount');
    } else if (val > w_amount) {
      dropDownAlertRef.alertWithType('error', 'Validation error', 'Your maximum wallet balance is ' + w_amount);
    } else {
    ...
    axios({
      ...
      .then(async response => {
        if (response.data.status == 1) {
          ...
          dropDownAlertRef.alertWithType('error', 'Validation error', response.data.message);
        }
      })
      .catch(error => {
        setLoading(false);
  ...
  const validate_amount = () => {
    if (w_amount > 0) {
      ...
      dropDownAlertRef.alertWithType('error', 'Validation error', 'Your balance is low');
    }
  }
