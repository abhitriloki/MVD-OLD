...
const EditAadharNumber = (props) => {
    ...
    const check_valid = () => {
        if (aadhar_number) {
            ...
            dropDownAlertRef.alertWithType('error', strings.validation_error, strings.please_enter_your_aadhar_number);
        }
    }
    ...
    const call_update_kyc = () => {
        ...
        axios({
            ...
            data: { driver_id: global.id, aadhar_number: aadhar_number }
        })
            .then(async response => {
                setLoading(false);
                dropDownAlertRef.alertWithType('success', strings.successfully_updated, strings.your_aadhar_number_has_been_updated);
                go_back();
            })
            .catch(error => {
                setLoading(false);
    ...
    return (
        <SafeAreaView style={{ backgroundColor: colors.lite_bg, flex: 1 }}>
            ...
            <View style={{ alignItems: 'center', justifyContent: 'center' }}>
                <Text numberOfLines={1} style={{ color: colors.theme_fg_two, fontSize: f_xl, fontFamily: bold }}>{strings.enter_your_aadhar_number}</Text>
                <View style={{ margin: 5 }} />
                <Text numberOfLines={1} style={{ color: colors.grey, fontSize: f_xs, fontFamily: normal }}>{strings.you_need_enter_your_aadhar_number}</Text>
                <View style={{ margin: 20 }} />
                <View style={{ width: '80%' }}>
...
export default EditAadharNumber;