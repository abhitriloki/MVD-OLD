...
const Earnings = (props) => {
  ...
  const call_get_earnings = (date) => {
    console.log({ date: date, id : global.id })
    setLoading(true);
    axios({
        method: 'post',
    ...
    > 
      ...
      />
        ...
        <View activeOpacity={1} style={{ width: '85%', alignItems: 'flex-start', justifyContent: 'center' }}>
          <Text numberOfLines={1} ellipsizeMode='tail' style={{ color: colors.theme_fg_three, fontSize: f_xl, fontFamily: bold }}>{strings.earnings}</Text>
        </View>
      </View>
          ...
          <DropShadow
            ...
            >
              <View style={{ backgroundColor:colors.theme_bg_three, width:'100%', padding:20, alignItems:'center', justifyContent:'center', borderRadius:10}}>
                <Text numberOfLines={1} style={{ color: colors.text_grey, fontSize: f_s, fontFamily:normal }}>{strings.total_earnings}</Text>
              ...
              <View style={{ backgroundColor:colors.theme_bg_three, width:'100%', padding:20, justifyContent:'center', borderRadius:10}}>
                <Text numberOfLines={1} style={{ color: colors.text_grey, fontSize: f_s, fontFamily:normal }}>{strings.trip_histories}</Text>
                <View style={{ margin:10 }} />
                {loading == true ?
                  ...
                  <View>
                    ...
                    :
                      <View style={{ justifyContent:'center', alignItems:'center' }}>
                        ...
                        <View style={{ margin:10 }} />
                        <Text numberOfLines={1} style={{ color: colors.text_grey, fontSize: f_s, fontFamily:normal }}>{strings.sorry_no_data_found}</Text>
                      </View>
                    }
                  </View>
                }
