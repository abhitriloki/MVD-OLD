...
const EditFirstName = (props) => {
    ...
    const check_valid = () => {
        if (first_name) {
            ...
            dropDownAlertRef.alertWithType('error', strings.validation_error, strings.please_enter_your_first_name);
        }
    }
    ...
    const call_profile_update = () => {
        ...
        axios({
            ...
            data: { driver_id: global.id, first_name: first_name }
        })
            .then(async response => {
                setLoading(false);
    ...
    return (
        <SafeAreaView style={{ backgroundColor: colors.lite_bg, flex: 1 }}>
            ...
            <View style={{ alignItems: 'center', justifyContent: 'center' }}>
                <Text numberOfLines={1} style={{ color: colors.theme_fg_two, fontSize: f_xl, fontFamily: bold }}>{strings.enter_your_first_name}</Text>
                <View style={{ margin: 5 }} />
                <Text numberOfLines={1} style={{ color: colors.grey, fontSize: f_xs, fontFamily: normal }}>{strings.you_need_enter_your_first_name}</Text>
                <View style={{ margin: 20 }} />
                <View style={{ width: '80%' }}>
